...
Reference for the `save-to-spotify` binary: installation, authentication, commands, flags, JSON mode, error handling, and common assistant workflows.
...
### Headless (remote server, CI, or Cline environment)
...
The CLI accepts both bare IDs and full Spotify URIs interchangeably, but assistants should prefer the full Spotify URI form whenever possible:
...
**Always use `--json` when operating as an assistant.** It must appear before the command:
