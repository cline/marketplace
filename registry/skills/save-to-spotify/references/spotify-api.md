...
- `https://developer.spotify.com/documentation/web-api/tutorials/building-with-ai` - Web API guidance for AI coding assistants and assistants.
...
`save-to-spotify token` prints the current access token to stdout. That token has been verified to work as a Spotify Web API `Bearer` token for requests against `api.spotify.com`, so assistants do not need a separate app registration for the catalog lookups below.
