...
The assistant passes `timeline.json` to `save-to-spotify --json timeline set --episode-id <EP_ID> --from-file timeline.json`. The CLI uploads each local image file to Spotify's image store and swaps the path for the returned upload token before PUT-ing the timeline.
...
2. The assistant writes: structured script broken into segments, with source URL(s) and image hint per segment
3. The assistant generates: one audio file per segment via TTS provider
4. The assistant generates: silence files for pauses/transitions
...
10. The assistant saves: save-to-spotify --json upload ...
11. Agent sets timeline: save-to-spotify --json timeline set ...
12. The assistant polls: episodes status until READY
```
