...
description: Analyze a codebase and recommend Cline automations (hooks, worker agents, skills, plugins, MCP servers). Use when user asks for automation recommendations, wants to optimize their Cline setup, mentions improving Cline workflows, asks how to first set up Cline for a project, or wants to know what Cline features they should use.
tools: Read, Glob, Grep, Bash
---

# Cline Automation Recommender

Analyze codebase patterns to recommend tailored Cline automations across all extensibility options.

**This skill is read-only.** It analyzes the codebase and outputs recommendations. It does NOT create or modify any files. Users implement the recommendations themselves or ask Cline separately to help build them.
...
| **Worker agents** | Specialized reviewers/analyzers that run in parallel |
| **Skills** | Packaged expertise, workflows, and repeatable tasks (invoked by Cline or user via `/skill-name`) |
...
# Check for existing Cline config
ls -la .cline/ AGENTS.md 2>/dev/null
...
| Testing | Jest, pytest, Playwright configs | Testing hooks, worker agents |
...
Create skills in `.cline/skills/<name>/SKILL.md`. Some are also available via plugins:
...
| Code style | **project-conventions** | Cline-only |
...
#### D. Worker agent Recommendations

See [references/subagent-templates.md](references/subagent-templates.md) for worker-agent templates.

| Codebase Signal | Recommended Worker agent |
|-----------------|---------------------|
...
**Install**: Configure `context7` in Cline MCP settings.
...
#### [skill name]
**Why**: [specific reason]
**Create**: `.cline/skills/[name]/SKILL.md`
**Invocation**: User-only / Both / Cline-only
...
- `user-invocable: false` — Cline-only (for background knowledge)
...
### When to Recommend Worker agents
...
Recommend headless Cline for automated pipelines:

```bash
# Pre-commit hook example
# Run your Cline-compatible automation entry point to fix lint errors in src/.

# CI pipeline with structured output
# Pipe structured output from your automation runner into your CI command.
```

### Permissions for Hooks

Configure allowed tools in `.cline/settings.json`:
