...
Hooks automatically run commands in response to Cline events. They're ideal for enforcement and automation that should happen consistently.
...
Notification hooks run when Cline sends notifications. Use matchers to filter by notification type.

### Permission Alerts
| Matcher | Use Case |
|---------|----------|
| `permission_prompt` | Alert when Cline requests permissions |
...
| Matcher | Use Case |
|---------|----------|
| `idle_prompt` | Alert when Cline is waiting for input (60+ seconds idle) |

**Recommend**: Play sound or send notification when Cline needs attention
**Value**: Know when Cline is ready for your input
    ...
    "Notification": [
      ...
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "osascript -e 'display notification \"Cline is waiting\" with title \"Cline\"'"
          }
        ]
      }
    ]
...
|---------|---------------|
| `permission_prompt` | Cline needs permission for a tool |
| `idle_prompt` | Cline waiting for input (60+ seconds) |
...
Hooks go in `.cline/settings.json`:
...
Recommend creating the `.cline/` directory if it doesn't exist.
