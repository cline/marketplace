...
Skills are packaged expertise with workflows, reference materials, and best practices. Create them in `.cline/skills/<name>/SKILL.md`. Skills can be invoked by Cline automatically when relevant, or by users directly with `/skill-name`.
...
| **agent-development** | Building specialized worker agents |
...
Create project-specific skills in `.cline/skills/<name>/SKILL.md`.
...
user-invocable: false           # Only Cline can invoke (for background knowledge)
allowed-tools: Read, Grep, Glob # Restrict tool access
context: fork                   # Run in isolated worker agent
...
3. Run validation: `bash ~/.cline/skills/create-migration/scripts/validate-migration.sh`
...
### Project Conventions (Cline-only)

Background knowledge Cline applies automatically:
...
The command output replaces the placeholder before Cline sees the skill content.
