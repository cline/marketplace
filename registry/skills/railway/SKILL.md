...
**Agent harness, human present**: when the CLI detects an agent harness (Cline or another coding-agent client, …) with a human at the keyboard, `railway up` opens the browser and skips the confirm prompt — the agent invocation is treated as consent. A real human still has to complete OAuth in the browser.
...
1. **Preferred — background execution** (use your agent harness's background command/session support, then poll output):
