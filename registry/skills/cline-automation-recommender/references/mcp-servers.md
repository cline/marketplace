...
MCP (Model Context Protocol) servers extend Cline's capabilities by connecting to external tools and services.
...
2. **Global config** (`~/.cline/mcp-settings.json`) - Available across all projects
...
### context7
**Best for**: Projects using popular libraries/SDKs where you want Cline to code with up-to-date documentation
...
**Value**: Cline fetches live documentation instead of relying on training data, reducing hallucinated APIs and outdated patterns.
...
**Value**: Cline can interact with your running app, take screenshots, fill forms, and verify UI behavior.
...
**Value**: Cline can query tables, manage auth, and interact with Supabase storage directly.
...
**Value**: Cline can introspect the live deployment (tables, function specs, env vars, logs) and execute queries/mutations against it via tools like `tables`, `function-spec`, `data`, `run-once-query`, `logs`, `env list/set/get`. Run via `npx convex mcp start`.
...
**Value**: Cline can create issues, review PRs, check workflow runs, and manage releases.
...
**Value**: Cline can investigate Sentry issues, find root causes, and suggest fixes.
...
**Value**: Cline remembers project context, decisions, and patterns across conversations.
