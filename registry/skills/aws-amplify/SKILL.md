...
1. **If an AWS MCP server or AWS documentation tools are available in Cline**, use them to search and retrieve relevant documentation pages.
2. **If those tools are unavailable**, fall back to Cline browsing/search tools or the `aws` CLI for resource verification.
