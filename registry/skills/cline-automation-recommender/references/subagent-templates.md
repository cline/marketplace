...
Worker agents are specialized Cline instances that run in parallel, each with their own context window and tool access. They're ideal for focused reviews, analysis, or generation tasks.

**Note**: These are common patterns. Design custom worker agents based on the codebase's specific review and analysis needs.
...
| If You See | Recommend Worker agent |
|------------|-------------------|
...
## Worker agent Placement

Worker agents go in `.cline/agents/`:
